...
class mod_switchcast_event {
     ...
     * Constructor
     *
     * @param mod_switchcast_series $a_obj_scast     SWITCHcast channel the clip belongs to
     * @param string                $clip_identifier clip ID as SWITCHcast
     * @param bool                  $isempty
     * @param int                   $switchcast_id
     */
    public function __construct(mod_switchcast_series $a_obj_scast, $clip_identifier, $isempty = false,
                                $switchcast_id = 0) {
        if (!$isempty) {
    ...
    public function fetch() {
        ...
        $event = mod_switchcast_apicall::sendRequest('/events/' . $this->getExtId() . '?withpublications=true', 'GET', null, false, true, null, false, true); // dont run-as, we want all events, we have filtered already
        $event_acls = mod_switchcast_apicall::sendRequest('/events/' . $this->getExtId() . '/acl', "GET", null, false, true, null, false, true); // dont run-as, we want all events, we have filtered already
        while ($acl = array_pop($event_acls)) {
            if ($acl->allow == true && $acl->action == 'write' && preg_match('/^ROLE\_AAI\_USER\_([^\_]+)$/', $acl->role,
                            $matches) && $matches[1] != mod_switchcast_series::getValueForKey('default_sysaccount')
            ) {
        ...
        $members = $DB->get_records('switchcast_cmember', ['clip_ext_id' => $this->getExtId()]);
    ...
    private function getUrlFor($event, $label = '') {
        ...
        foreach ($event->publications as $publication) {
            switch ($label) {
                ...
                case 'Streaming':
                    if ($publication->channel == mod_switchcast_series::getValueForKey('pubchannel_videoplayer')) {
                        $url = $publication->url;
                    }
                    break;

                case 'Download':
                    if ($publication->channel == mod_switchcast_series::getValueForKey('pubchannel_download')) {
                ...
                case 'Annotate':
                    if ($publication->channel == mod_switchcast_series::getValueForKey('pubchannel_annotate')) {
                        $url = $publication->url;
                ...
                case 'Cover image':
                    $preview_urls = [];
                    $thumbnail_flavors = explode(',', mod_switchcast_series::getValueForKey('thumbnail_flavors'));
        ...
        if ($url) {
            ...
            $link = $CFG->wwwroot . '/mod/switchcast/go_to.php';
            $link .= '?url=' . base64_encode($url);
            $link .= '&salt=' . base64_encode($salt);
            $link .= '&token=' . base64_encode(sha1(mod_switchcast_series::getValueForKey('default_sysaccount') . $salt . $this->switchcast_id . $url));
    ...
    public function update() {
        ...
        mod_switchcast_apicall::sendRequest('/events/' . $this->getExtId() . '/metadata?type=dublincore/episode', 'PUT',
                $event_metadata);

        // second, update ACLs with (possibly) new owner :
        // 1.- get current ACLs
        $event_acls = mod_switchcast_apicall::sendRequest('/events/' . $this->getExtId() . '/acl', 'GET', null, false, true, null, false, true);
        ...
        mod_switchcast_apicall::sendRequest('/events/' . $this->getExtId() . '/acl', 'PUT', $data);
    ...
    public function delete() {
        ...
        $sysaccount = mod_switchcast_series::getSysAccountByOrganization($this->series->getOrganization(), true);

        if (!$sysaccount) {
            return false;
        }

        // TODO SOMETIME : make sure we have write access to this clip

        $url = '/events/' . $this->getExtId();

        mod_switchcast_apicall::sendRequest($url, "DELETE");

        $DB->delete_records('switchcast_cmember', ['clip_ext_id' => $this->getExtId()]);
    ...
    public function isAllowed($perm_type) {
        ...
        $mod_switchcast_user = new mod_switchcast_user();
        $user_uploaded_events = $DB->get_records('switchcast_uploadedclip', ['userid' => $USER->id]);
        ...
        if ($perm_type == 'write') {
            if (has_capability('mod/switchcast:isproducer',
                            $context) || (($mod_switchcast_user->getExternalAccount() == $this->getOwner()) && $this->getOwner() !== '') || in_array($this->getExtId(),
                            $user_uploaded_events_extids)
            ) {
                /*
            ...
            if ((has_capability('mod/switchcast:isproducer', $context)) || (has_capability('mod/switchcast:seeallclips',
                            $context)) || ($this->series->getIvt() && $this->getOwner() !== '' && ($mod_switchcast_user->getExternalAccount() == $this->getOwner())) || ($this->series->getIvt() == false) || ($this->series->getIvt() == true && $this->series->getInvitingPossible() == true && is_numeric(array_search($USER->id,
                                    $this->getMembers()))) || (mod_switchcast_user::checkSameGroup(mod_switchcast_user::getMoodleUserIdFromExtId($this->getOwner()),
     ...
     */
    public function addMember($userid, $courseid, $switchcastid) {
        global $DB, $context;
        ...
        if (!has_capability('mod/switchcast:use', $context, $userid)) {
            return false;
        }
        ...
        $new_record->switchcastid = $switchcastid;
        $DB->insert_record('switchcast_cmember', $new_record);
     ...
     * @param     $switchcastid
     */
    public function removeMember($userid, $courseid, $switchcastid) {
        global $DB;
        $DB->delete_records('switchcast_cmember', [
                'clip_ext_id'  => $this->getExtId(), 'userid' => $userid, 'courseid' => $courseid,
                'switchcastid' => $switchcastid
        ]);
        $newmembers = [];
