...
class mod_switchcast_apicall {
    ...
    static function sendRequest($url, $request_type, $data = null, $return_rawdata = false, $usecache = true,
        ...
        $request_url = mod_switchcast_series::getValueForKey('switch_api_host') . $url;
        $cache_time = mod_switchcast_series::getValueForKey('local_cache_time');
        ...
        if ($request_type !== 'GET') {
            ...
            if (preg_match('/\/series\/([0-9a-zA-Z]+)/', $request_url, $matches)) {
                ...
                mod_switchcast_log::write("CACHE : destroying cache for series " . $matches[1] . " because " . $reason);
                ...
                mod_switchcast_log::write("CACHE : destroying entire cache because " . $reason);
                self::clear_cache($cache_dir);
            }
        }
        if (!file_exists($cache_dir)) {
            mod_switchcast_log::write("CACHE : initializing empty cache");
            mkdir($cache_dir);
        }
        ...
        mod_switchcast_log::write("REQUEST " . $request_type . " " . $request_url);
        mod_switchcast_log::write("INPUT " . print_r($data, true));
        ...
        if ($usecache && (string)$request_type === 'GET' && $cache_time && $cache_dir && file_exists($cache_filename) && (time() - filemtime($cache_filename) < $cache_time)) {
            ...
            mod_switchcast_log::write("CACHE : using cached file " . $cache_filename);
            $output = file_get_contents($cache_filename);
        }
        else {
            // no cache for this request
            mod_switchcast_log::write("CACHE : no cached file");
            ...
            if ($runas) {
                $role_user = 'ROLE_AAI_USER_' . mod_switchcast_user::getExtIdFromMoodleUserId($USER->id);
            ...
            if (isset($file)) {
                if (!filesize($file) || !is_readable($file)) {
                    mod_switchcast_log::write("CURL UPLOAD ERROR : empty or unreadable file");
                    if ($haltonerror) {
                ...
                //                curl_setopt($curl_request, CURLOPT_VERBOSE, (bool)mod_switchcast_obj::getValueForKey('logging_enabled'));
            }
            else {
                curl_setopt($curl_request, CURLOPT_TIMEOUT_MS,
                        (int)mod_switchcast_series::getValueForKey('curl_timeout') * 1000);
            }

            curl_setopt($curl_request, CURLOPT_CAINFO, mod_switchcast_series::getValueForKey('cacrt_file'));
            curl_setopt($curl_request, CURLOPT_SSL_VERIFYPEER, false);

            // TODO WAIT FOR SWITCH : no signing for now (test phase)
            curl_setopt($curl_request, CURLOPT_USERPWD, 'apitest:apitest'); // for unil-ch.producer
            //            curl_setopt($curl_request, CURLOPT_USERPWD, 'apitest:IE8hIvOZrNr0Fzqr'); // for unil-ch.producer
            //            curl_setopt($curl_request, CURLOPT_SSLCERT, mod_switchcast_obj::getValueForKey('crt_file'));
            //            curl_setopt($curl_request, CURLOPT_SSLKEY, mod_switchcast_obj::getValueForKey('castkey_file'));
            //            if(mod_switchcast_obj::getValueForKey('castkey_password')) {
            //                curl_setopt($curl_request, CURLOPT_SSLKEYPASSWD, mod_switchcast_obj::getValueForKey('castkey_password'));
            //            }

            curl_setopt($curl_request, CURLOPT_URL, $request_url);
            if (mod_switchcast_series::getValueForKey('curl_proxy')) {
                curl_setopt($curl_request, CURLOPT_PROXY, mod_switchcast_series::getValueForKey('curl_proxy'));
            }
            if (is_array($data)) {
                $postfields = [];
                if ($file) {
                ...
                foreach ($data as $key => $value) {
                    $postfields[$key] = htmlentities($value, ENT_NOQUOTES);
                }
            ...
            if (isset($file)) {
                if ($curl_errno) {
                    ...
                    mod_switchcast_log::write("CURL UPLOAD ERROR : no. " . $curl_errno);
                    mod_switchcast_log::write("                  : " . $output);
                    if ($haltonerror) {
                        throw new moodle_exception('uploaderror', 'switchcast');
                    }
            ...
            if ($output && (string)$request_type === 'GET' && (isset($response_details) && $response_details['http_code'] < 400) && ($cache_time && $cache_dir && is_writable($cache_dir))) {
                ...
                mod_switchcast_log::write("CACHE : writing output to cache file " . $cache_filename);
        ...
        if ($output === false) {
            if ($curl_errno) {
                mod_switchcast_log::write("CURL REQUEST ERROR : no. " . $curl_errno);
            }
            if ($haltonerror) {
        ...
        mod_switchcast_log::write("OUTPUT " . $output);
        ...
        try {
            ...
            if ($haltonerror) {
                print_error('api_fail', 'switchcast', null, $e->getMessage() . $e->getCode());
            }
        ...
        if (isset($response_details) && $response_details['http_code'] >= 400) {
            if ($response_details['http_code'] == 404) {
                if ($haltonerror) {
                    print_error('api_404', 'switchcast', null, $response_details['http_code']);
                }
            }
            if ($haltonerror) {
                print_error('api_fail', 'switchcast', null, $response_details['http_code']);
            }
    ...
    static function hashfilename($url = '') {
        $f = str_replace(mod_switchcast_series::getValueForKey('switch_api_host'), '', $url);
        $f = str_replace(mod_switchcast_series::getValueForKey('default_sysaccount'), '', $f);
