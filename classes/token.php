...
class mod_switchcast_token {
    ...
    static function ext_auth_decode_encrypted_token($encrypted_token_base64) {
        ...
        $private_key = openssl_get_privatekey('file://' . scast_obj::getValueByKey('serverkey_file'),
                scast_obj::getValueByKey('serverkey_password'));
        if ($private_key === false) {
            print_error('error_opening_privatekey', 'switchcast', '',
                    'file:/' . scast_obj::getValueByKey('serverkey_file'));
        }
        ...
        if (count($parts) == 3) {
            return ['channel_id' => $parts[0], 'clip_id' => $parts[1], 'plain_token' => $parts[2]];
        }
        else {
