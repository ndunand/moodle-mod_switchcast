...
class mod_switchcast_series {
    ...
    public static function getMaxfilesizes($enforceglobalmax = false) {
        ...
        if ($enforceglobalmax) {
            ...
            $globalmax = mod_switchcast_series::getValueForKey('userupload_maxfilesize');
    ...
    public function createClip($options) {
        ...
        $data = [
                ...
                // see https://dokuwiki.toolbox.switch.ch/switchcast-api/api_conventions

        ];

        return mod_switchcast_apicall::sendRequest($url, 'POST', $data, null, null, $options['filename']);
    }
    ...
    public static function processUploadedClips() {
        ...
        $stale_records = $DB->get_records_select('switchcast_uploadedclip',
                'status = ' . SWITCHCAST_CLIP_UPLOADED . ' AND timestamp < ' . $staletime);
        foreach ($stale_records as $stale_record) {
            ...
            if ($user_stale) {
                ...
                email_to_user($user_stale, $admin, get_string('clipstale_subject', 'switchcast'),
                        get_string('clipstale_body', 'switchcast', $a_s));
                // notify admin too
                $a_s->userlink = $CFG->wwwroot . '/user/profile.php?id=' . $user_stale->id;
                $a_s->userfullname = fullname($user_stale);
                email_to_user($admin, $admin, get_string('clipstale_subject_admin', 'switchcast'),
                        get_string('clipstale_body_admin', 'switchcast', $a_s));
            }
        }
        $DB->delete_records_select('switchcast_uploadedclip',
                'status = ' . SWITCHCAST_CLIP_UPLOADED . ' AND timestamp < ' . $staletime);
        ...
        foreach ($switchcasts as $switchcast) {
            ...
            $uploaded_videos = $DB->get_records('switchcast_uploadedclip', ['switchcastid' => $switchcast->id]);
            if (!$uploaded_videos) {
                continue;
            }

            $series = new mod_switchcast_series();
            try {
                // try and fetch the series on the back-end BUT do not halt on error
                $fetch_result = $series->fetch($switchcast->id, true, true, false);
                if ($fetch_result == false) {
                    throw new moodle_exception('api_404', 'switchcast');
                }
            }
            catch (Exception $e) {
            ...
            foreach ($uploaded_videos as $uploaded_video) {
                if ($uploaded_video->status == SWITCHCAST_CLIP_READY) {
                    // encoding finished
                    if (!in_array($uploaded_video->ext_id, $series_event_indentifiers)) {
                        // clip deleted
                        $DB->delete_records('switchcast_uploadedclip', ['id' => $uploaded_video->id]);
                    }
                    else {
                    ...
                    foreach ($series_events as $series_event) {
                        if ($series_event->identifier == $uploaded_video->ext_id) {
                            if ($series_event->processing_state == SWITCHCAST_PROCESSING_SUCCEEDED && count($series_event->publications)) {
                                // it's ready!
                                $uploaded[] = $uploaded_video->filename;
                                $uploaded_video->status = SWITCHCAST_CLIP_READY;
                                $DB->update_record('switchcast_uploadedclip', $uploaded_video);
                                ...
                                if ($user !== false) {
                                    ...
                                    email_to_user($user, $admin, get_string('clipready_subject', 'switchcast'),
                                            get_string('clipready_body', 'switchcast', $a));
                                }
                            }
                        }
                    }
    ...
    public function addProducer($aaiUniqueId, $usesysaccount = true) {
        ...
        $ch_acls = mod_switchcast_apicall::sendRequest($url, 'GET', null, false, true, null, false);
        ...
        mod_switchcast_apicall::sendRequest($url, 'PUT', $data, false, true, null, false);
    ...
    public static function getValueForKey($key) {
        return get_config('switchcast', str_replace('.', 'DOT', $key));
    }
    ...
    public function removeProducer($aaiUniqueId) {
        ...
        $ch_acls = mod_switchcast_apicall::sendRequest($url, 'GET');
        ...
        $data = ['acl' => json_encode($ch_acls, JSON_UNESCAPED_SLASHES)];
        mod_switchcast_apicall::sendRequest($url, 'PUT', $data);
    ...
    public function getEvents($filters = []) {
        ...
        $events = mod_switchcast_apicall::sendRequest($url, 'GET', null, false, true, null, false, true); // dont run-as, we want all events, we'll be filtering later

        $count_events = count($events);
        for ($i = 0; $i < $count_events; $i++) {
            if (count($events[$i]->publications) == 0) {
            ...
            foreach ($filters as $filter_key => $value) {
                ...
                if (in_array($filter_key, ['title', 'location', 'presenter'])) {
                    // TODO WAIT FOR SWITCH : check if works when "presenter" bug is resolved, cf. https://dokuwiki.toolbox.switch.ch/switchcast-api/api_known_issues
                    ...
                    $event = new mod_switchcast_event($this, $events[$i]->identifier);
                    $event->fetch();
    ...
    public function checkAccess($clips = []) {
        ...
        foreach ($clips as $clip) {
            // we have to instantiate a new mod_switchcast_clip because $clip is a SimpleXMLElement
            if (!isset($allclips[(string)$clip->identifier])) {
                $clipobj = new mod_switchcast_event($this, (string)$clip->identifier, false, $switchcast->id);
    ...
    function doCreate() {
        ...
        if ($this->getExtId() == '') {
            ...
            $new_series = mod_switchcast_apicall::sendRequest($url, 'POST', $data);
    ...
    function fetch($id, $inmoodle = true, $returninfo = false, $haltonerror = true) {
        ...
        if ($inmoodle) {
            ...
            if (is_number($id)) {
                $rec = $DB->get_record('switchcast', ['id' => $id]);
            }
            else {
                $rec = $DB->get_record('switchcast', ['ext_id' => $id]);
            }
        ...
        $ch = mod_switchcast_apicall::sendRequest($url, 'GET', null, false, true, null, false, $haltonerror);
        if (!$ch && !$haltonerror) {
            // no need to go further
            return false;
        }

        $ch_metadata = mod_switchcast_apicall::sendRequest($url . '/metadata', 'GET', null, false, true, null, false);
        $ch_acls = mod_switchcast_apicall::sendRequest($url . '/acl', 'GET', null, false, true, null, false);
...
//        return true;
        ...
        mod_switchcast_apicall::sendRequest($url, 'PUT', $data); // will break on error
    ...
    public function getExternalAuthName($id = 0) {
        ...
        $new = mod_switchcast_oldapi::sendRequest($url, 'GET');
    ...
    public function getAllLicenses() {
        ...
        if (!$this->getExtId()) {
            ...
            $licenses = [
                    "ALLRIGHTS"   => "EVENTS.LICENSE.ALLRIGHTS", "CC-BY" => "EVENTS.LICENSE.CCBY",
                    "CC-BY-NC"    => "EVENTS.LICENSE.CCBYNC", "CC-BY-NC-ND" => "EVENTS.LICENSE.CCBYNCND",
                    "CC-BY-NC-SA" => "EVENTS.LICENSE.CCBYNCSA", "CC-BY-ND" => "EVENTS.LICENSE.CCBYND",
                    "CC-BY-SA"    => "EVENTS.LICENSE.CCBYSA", "CC0" => "EVENTS.LICENSE.CC0"
            ];
        }
        else {
            $series_metadata =
                    mod_switchcast_apicall::sendRequest('/series/' . $this->getExtId() . '/metadata?type=dublincore/series',
                            'GET');
            $licenses = self::search_collection($series_metadata, 'id', 'license', 'collection');
        }

        foreach ($licenses as $key => &$value) {
            $value = get_string("license_" . $value, 'switchcast');
        }
    ...
    public function getAllReferences($ext_id = false) {
        ...
        $records = $DB->get_records('switchcast', ['ext_id' => $ext_id]);
