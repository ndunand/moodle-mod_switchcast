...
class xoctPublicationUsageFormGUI extends ilPropertyFormGUI {
    ...
    public function fillForm() {
        $array = [
                self::F_USAGE_ID       => $this->object->getUsageId(), self::F_TITLE => $this->object->getTitle(),
                self::F_DESCRIPTION    => $this->object->getDescription(),
                self::F_PUBLICATION_ID => $this->object->getPublicationId(),
                self::F_EXT_ID         => $this->object->getExtId(), self::F_MD_TYPE => $this->object->getMdType(),
        ];
    ...
    public function saveObject() {
        ...
        if (!xoctPublicationUsage::where(['usage_id' => $this->object->getUsageId()])->hasSets()) {
            $this->object->create();
        }
        else {
            $this->object->update();
    ...
    protected function initButtons() {
        if ($this->is_new) {
            ...
            $this->addCommandButton(xoctPublicationUsageGUI::CMD_CREATE,
                    $this->parent_gui->txt(xoctPublicationUsageGUI::CMD_CREATE));
        }
        else {
            $this->setTitle($this->parent_gui->txt('edit'));
            $this->addCommandButton(xoctPublicationUsageGUI::CMD_UPDATE,
                    $this->parent_gui->txt(xoctPublicationUsageGUI::CMD_UPDATE));
        }

        $this->addCommandButton(xoctPublicationUsageGUI::CMD_CANCEL,
