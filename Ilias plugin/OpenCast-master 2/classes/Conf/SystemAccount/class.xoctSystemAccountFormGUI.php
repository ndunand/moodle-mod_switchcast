...
class xoctSystemAccountFormGUI extends ilPropertyFormGUI {
    ...
    public function fillForm() {
        $array = [
                self::F_DOMAIN => $this->object->getDomain(), self::F_EXT_ID => $this->object->getExtId(),
        ];
    ...
    public function saveObject() {
        ...
        if (!xoctSystemAccount::where(['domain' => $this->object->getDomain()])->hasSets()) {
            $this->object->create();
        }
        else {
            $this->object->update();
    ...
    protected function initButtons() {
        if ($this->is_new) {
            ...
            $this->addCommandButton(xoctSystemAccountGUI::CMD_CREATE,
                    $this->parent_gui->txt(xoctSystemAccountGUI::CMD_CREATE));
        }
        else {
            $this->setTitle($this->parent_gui->txt('edit'));
            $this->addCommandButton(xoctSystemAccountGUI::CMD_UPDATE,
                    $this->parent_gui->txt(xoctSystemAccountGUI::CMD_UPDATE));
        }

        $this->addCommandButton(xoctSystemAccountGUI::CMD_CANCEL,
                $this->parent_gui->txt(xoctSystemAccountGUI::CMD_CANCEL));
    }
}
