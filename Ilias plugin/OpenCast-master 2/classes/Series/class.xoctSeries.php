...
class xoctSeries extends xoctObject {
    ...
    public function create() {
        ...
        $array['metadata'] = json_encode([$this->getMetadata()->__toStdClass()]);
        $unibe = new xoctAcl();
        $unibe->setRole('ROLE_UNIBE.CH_MEMBER');
        $unibe->setAllow(true);
        $unibe->setAction(xoctAcl::READ);
        $array['acl'] = json_encode([
                xoctAcl::userRead(), xoctAcl::adminWrite(), xoctAcl::adminWrite(), $unibe->__toStdClass()
        ]);
    ...
    public function update() {
        ...
        xoctRequest::root()->series($this->getIdentifier())->metadata()
                ->parameter('type', $this->getMetadata()->getFlavor())->put($array);
