...
class ilObjOpenCastGUI extends ilObjectPluginGUI {
    ...
    public function executeCommand() {
        try {
            if ($this->access->checkAccess('read', '', $_GET['ref_id'])) {
                ...
                $this->history->addItem($_GET['ref_id'], $this->ctrl->getLinkTarget($this, $this->getStandardCmd()),
                        $this->getType(), '');
            }
            ...
            $this->tpl->setTitleIcon(ilUtil::getImagePath('icon_xoct.svg',
    ...
    protected function setTabs() {
        $this->tabs_gui->addTab(self::TAB_EVENTS, $this->pl->txt('tab_event_index'),
                $this->ctrl->getLinkTarget(new xoctEventGUI(), xoctEventGUI::CMD_STANDARD));
        $this->tabs_gui->addTab(self::TAB_INFO, $this->pl->txt('tab_info'),
                $this->ctrl->getLinkTarget($this, 'infoScreen'));
        $this->tabs_gui->addTab(self::TAB_SETTINGS, $this->pl->txt('tab_series_settings'),
                $this->ctrl->getLinkTarget(new xoctSeriesGUI(), xoctSeriesGUI::CMD_EDIT));
        $this->tabs_gui->addTab(self::TAB_GROUPS, $this->pl->txt('tab_groups'),
                $this->ctrl->getLinkTarget(new xoctGroupGUI()));
    ...
    public function afterSave(ilObjOpenCast $newObj, $additional_args) {
        ...
        if (xoctOpenCast::where(['obj_id' => $newObj->getId()])->hasSets()) {
            $cast->update();
        }
        else {
            $cast->create();
