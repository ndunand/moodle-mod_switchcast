...
$url = new moodle_url('/mod/switchcast/clip_editdetails.php', array('id' => $id, 'clip_ext_id' => $clip_ext_id));
...
$renderer->display_clip_outline($sc_clip, false, null, null, true);
echo html_writer::end_tag('table');

if ( has_capability('mod/switchcast:isproducer', $context) ) {
    echo html_writer::start_tag('form', array('method' => 'post', 'action' => 'clip_editdetails.php', 'class' => 'details border'));
    echo html_writer::input_hidden_params($url, array('action', 'userid'));
    echo html_writer::empty_tag('input', array('type' => 'hidden', 'name' => 'sesskey', 'value' => sesskey()));
    echo html_writer::empty_tag('input', array('type' => 'hidden', 'name' => 'action', 'value' => 'edit'));
    echo html_writer::start_div();
    echo html_writer::tag('span', get_string('title', 'switchcast'));
    echo html_writer::empty_tag('input', array('type' => 'text', 'name' => 'title', 'value' => $sc_clip->getTitle()));
    echo html_writer::end_div();
    echo html_writer::start_div();
    echo html_writer::tag('span', get_string('subtitle', 'switchcast'));
    echo html_writer::empty_tag('input', array('type' => 'text', 'name' => 'subtitle', 'value' => $sc_clip->getSubtitle()));
    echo html_writer::end_div();
    echo html_writer::start_div();
    echo html_writer::tag('span', get_string('presenter', 'switchcast'));
    echo html_writer::empty_tag('input', array('type' => 'text', 'name' => 'presenter', 'value' => $sc_clip->getPresenter()));
    echo html_writer::end_div();
    echo html_writer::start_div();
    echo html_writer::tag('span', get_string('location', 'switchcast'));
    echo html_writer::empty_tag('input', array('type' => 'text', 'name' => 'location', 'value' => $sc_clip->getLocation()));
    echo html_writer::end_div();
    echo html_writer::start_div();
    echo html_writer::tag('span', get_string('owner', 'switchcast'));
    $renderer->display_user_selector(true, 'clip_editdetails.php', get_string('setnewowner', 'switchcast'), true, true, true, $sc_clip->getOwnerUserId());
    echo html_writer::end_div();
    echo html_writer::start_div();
...
if ( $USER->id == $sc_clip->getOwnerUserId() || has_capability('mod/switchcast:isproducer', $context) ) {
    $renderer->display_user_selector(true, 'clip_editdetails.php', get_string('setnewowner', 'switchcast'), true, true);
    echo html_writer::start_tag('form', array('method' => 'post', 'action' => 'clip_editdetails.php', 'class' => 'border'));
    echo html_writer::input_hidden_params($url, array('action', 'userid'));
    echo html_writer::empty_tag('input', array('type' => 'hidden', 'name' => 'sesskey', 'value' => sesskey()));
