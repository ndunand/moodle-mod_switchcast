...
$url = new moodle_url('/mod/switchcast/upload_event.php', ['id' => $id]);
...
$maxbytes = min(mod_switchcast_series::getValueForKey('userupload_maxfilesize'), $switchcast->userupload_maxfilesize);
...
$mform = new mod_switchcast_upload_form($url, ['data' => $data, 'options' => $options]);

if ($mform->is_cancelled()) {
    redirect($return_channel);
}
else if ($formdata = $mform->get_data()) {
    $formdata = file_postupdate_standard_filemanager($formdata, 'files', $options, $usercontext, 'mod_switchcast',
            'userfiles', $id);
    $fs = get_file_storage();
    ...
    foreach ($files as $file) {
        ...
        if (!in_array(strtolower($extension[1]), mod_switchcast_series::getAllowedFileExtensions())) {
            $file->delete();
            $a = new stdClass();
            $a->yours = $extension[1];
            $a->allowed = implode(', ', mod_switchcast_series::getAllowedFileExtensions());
        ...
        try {
            $result = $sc_obj->createClip([
                    ...
                    'ivt__owner' => mod_switchcast_user::getExtIdFromMoodleUserId($USER->id),
...
if (isset($formdata) && isset($result)) {
    ...
    $upl_a = mod_switchcast_series::getValueForKey('uploadfile_extensions');
